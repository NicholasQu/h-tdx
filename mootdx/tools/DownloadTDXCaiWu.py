...
class DownloadTDXCaiWu(object):
    tdx_root_dir = 'D:/new_tdx'
    tdx_cw_dir = tdx_root_dir + '/vipdoc/cw';
    tmp_cw_dir = 'D:/cw_tmp' #所有的文件处理都在这个文件夹下，不修改tdx本地的财务文件，确认无误后再同步回tdx目录下
    ...
    def download_file(self, url, save_dir, save_file_name):
    ...
    def checksum(self, hash_expected, check_file_name, check_file_dir, is_eq):
    ...
    def check_hashlist(self, hash_file_name, check_file_dir, is_eq, file_name_scope=[]):
        ...
        for file_name in self.workerPool.map(self.checksum, list(file_hashmap.values()), list(file_hashmap.keys()), repeat(check_file_dir), repeat(is_eq)):
            if file_name is not None and len(file_name)>0:
                to_update_cwfiles.append(file_name)

        return to_update_cwfiles;
    ...
    def copy_cw_to_tdx(self, copy_files_list, src_dir, dst_dir):
